      ...
      if (parenRange) {
        ...
        if (e.key === 'Backspace') {
          ...
          if (!hasSel && cur === pS) {
            ...
            if (cur > 0 && rawName.length > 0) {
              const newVal = rawName.slice(0, -1)
              setEditVal(newVal)
              liveUpdate(newVal)
              setTimeout(() => { if (inputRef.current) { inputRef.current.selectionStart = newVal.length; inputRef.current.selectionEnd = newVal.length } }, 0)
            }
            return
          }
